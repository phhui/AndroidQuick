	...
	chplayer.prototype = {
		...
		*/
		getVideo: function() {
			...
			if(this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				var html = '';
				if(!this.isM3u8) {
					html = '<video id="' + vid + '"' + src + ' width="100%" height="100%"' + autoplay + loop + ' webkit-playsinline playsinline x5-playsinline x5-video-player-type="h5">' + source + track + '</video>';
				} else {
					html = '<video id="' + vid + '" width="100%" height="100%"' + loop + ' webkit-playsinline playsinline x5-playsinline x5-video-player-type="h5">' + track + '</video>';
				}
		...
		*/
		addVEvent: function() {
			...
			var eventVideoClick = function(event) {
				//alert("点击 "+event.clientX+","+event.clientY);
				if(event.clientX<120&&event.clientY<120){
					//alert("点击关闭");
					thisTemp.pause();
					closeVideo();
				}else thisTemp.videoClick();
			};
			this.addListener('click', eventVideoClick);
			this.addListener('click', eventVideoClick, this.MDC);
			if(this.closeBtn)this.closeBtn.addEventListener('click',eventVideoClick);
			else{
				setTimeout(function(){
					this.closeBtn.addEventListener('click',eventVideoClick);
				},1000);
			}
			//延迟计算加载失败事件
			this.timerErrorFun();
		...
		*/
		...
		interFace: function() {
			...
			*/
			...
			//this.getByElement(closeBtnId).innerHTML=this.closeBtn;
			...
			this.CB = {
				...
				closeBtn:this.getByElement(closeBtnId,pd),
			...
			this.css(closeBtnId, {
				...
				height: '31px',
				overflow: 'hidden',
				'background-image':"url('./css/goback.png')",
		...
		*/
		endedHandler: function() {
			if(!this.vars['loop']) {
				this.pause();
				closeVideo();
				EventHelper.call("js_video_play_end");
			}
		},
		/*
			内部函数
		...
		*/
		...
		-----------------------------------------------------------------------------接口函数开始
			...
			在播放和暂停之间切换
		*/
		playOrPause: function(play=true) {
